...
import { ANTIGRAVITY_DEFAULT_PROJECT_ID, ANTIGRAVITY_ENDPOINT_FALLBACKS, ANTIGRAVITY_ENDPOINT_PROD, ANTIGRAVITY_ENDPOINT_DAILY_NONSANDBOX, ANTIGRAVITY_PROVIDER_ID, getAntigravityHeaders, } from "./constants";
...
import { buildThinkingWarmupBody, } from "./plugin/request";
...
function getOAuthCallbackTimeoutMs() {
    ...
    return Math.min(MAX_OAUTH_CALLBACK_TIMEOUT_MS, Math.max(MIN_OAUTH_CALLBACK_TIMEOUT_MS, parsed));
}
    ...
    return {
        ...
        auth: {
            ...
            loader: async (getAuth, provider) => {
                ...
                let cachedAgyAuthState = null;
                let cachedAgyBinaryPath = null;
                return {
                    apiKey: "",
                        ...
                        if (!transport.auth.requiresOAuth) {
                            ...
                            try {
                                ...
                                if (transport.id === "cli") {
                                    ...
                                    const command = prepared.transportPayload;
                                ...
                                const result = await (async () => {
                                    ...
                                    if (listener) {
                                        try {
                                            ...
                                            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("SOFT_TIMEOUT")), softTimeoutMs));
                                            let callbackUrl;
                                            try {
                        ...
                        if (listener) {
                            return {
                                ...
                                callback: async () => {
                                    ...
                                    try {
                                        const callbackPromise = listener.waitForCallback();
                                        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("CALLBACK_TIMEOUT")), callbackTimeoutMs));
                                        let callbackUrl;
                                        try {
                                            callbackUrl = await Promise.race([callbackPromise, timeoutPromise]);
                                        }
